...
Router.post(
  ...
  async (request: Request, response: Response): Promise<Response> => {
    ...
    if (
      typeof email == "string" &&
      email != "" &&
      typeof email_cfg == "string" &&
      email_cfg != "" &&
      typeof password == "string" &&
      password != "" &&
      typeof password_cfg == "string" &&
      password_cfg != ""
    ) {
      if (email != email_cfg || password != password_cfg) {
        return response
          .status(500)
          .json({ msg: "Email or password confirmation are not valid !" });
      }
      ...
      try {
        ...
        return response
          .status(500)
          .json({ msg: "Error while creating the user !" });
      }
    } else {
      return response
        .status(500)
        .json({ msg: "You have to send email, password and confirmations !" });
    }
  }
);
...
Router.post(
  "/login",
  async (request: any, response: Response): Promise<Response> => {
    ...
    if (
      ...
      return response
        .status(500)
        .json({ msg: "You have to send email and password !" });
    }
  }
);
